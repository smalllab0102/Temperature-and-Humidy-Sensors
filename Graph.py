...
# Clean up date formatting on the X-axis (HH:MM:SS)
for ax in axes:
...
def update(frame):
    ...
    for ax, graphs, ydata in zip(axes, graph, y):
        ...
        # Explicitly force the X-axis limits to match our 24-hour window data
        ax.set_xlim(x.min(), x.max())
        
        ax.relim()
        ax.autoscale_view(scalex=False, scaley=True) # Scale Y automatically, keep X bounded
